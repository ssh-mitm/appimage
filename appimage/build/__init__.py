...
class BuildConfig:
    ...
    build_pylock : str
        ...
        unpinned and unverified, on every build — a gap ``pylock`` does
        not cover, since ``--only-deps`` deliberately excludes the local
        project when generating it. When set, this file is installed with
        ``pip install --require-hashes`` before the local project itself
        is installed with ``--no-build-isolation``, so the isolated build
        environment pip would otherwise create is skipped in favor of the
        already-verified backend. Generated by ``--lock`` alongside
        ``pylock`` (see docs/reproducible-builds.md) — not something
        hand-written.
...
def _run_pip_lock(
    ...
    *,
    ...
    which requirements go in and whether ``--only-deps`` applies. Run
    through *python_bin*, the same bundled python-build-standalone
    interpreter the real build installs into, so the resolved
    wheels/hashes match that exact platform and Python build rather than
    whatever the developer's own interpreter would resolve.
    """
    ...
    cmd = [str(python_bin), "-m", "pip", "lock", *requirements, *extra_args, "-o", str(output_path)]
...
def _generate_lock(
    ...
    *,
    ...
    _run_pip_lock(
        python_bin, project_root, resolved.install_targets, pylock_path,
        uploaded_prior_to=uploaded_prior_to, extra_args=("--only-deps",),
    )
    return pylock_path
...
def _generate_build_pylock(
    ...
    *,
    ...
    _run_pip_lock(python_bin, project_root, requires, build_pylock_path, uploaded_prior_to=uploaded_prior_to)
    return build_pylock_path


def _write_lock_config(pyproject_path: Path, project_root: Path, key: str, path: Path) -> None:
    """Write a generated lock file's path into ``[tool.appimage.build]`` if not already set."""
    with pyproject_path.open("rb") as f:
        data = tomllib.load(f)
    if key in data.get("tool", {}).get("appimage", {}).get("build", {}):
        return
...
def lock(config: BuildConfig, project_root: Path, *, uploaded_prior_to: str = "") -> None:
    ...
    ``build_pylock`` into ``[tool.appimage.build]`` for whichever of the
...
def _pip_env(python_bin: Path) -> dict[str, str]:
    """Environment for a pip subprocess run through *python_bin*.

    Prepends the interpreter's own ``bin/`` directory to ``PATH`` so a
    console-script entry point pip just installed into it (e.g. the
    ``uv-build`` binary backing the ``uv_build`` package, looked up via
    ``shutil.which`` inside its own build-backend shim) is found by a
    later ``pip install --no-build-isolation`` in the same interpreter —
    the subprocess otherwise only inherits this process's own ``PATH``,
    which has no reason to include a freshly-extracted interpreter's bin
    directory.
    """
    return {**os.environ, "PATH": f"{python_bin.parent}{os.pathsep}{os.environ.get('PATH', '')}"}


def _install_build_pylock(resolved: _ResolvedBuild, python_bin: Path, project_root: Path) -> list[str]:
    """Hash-verify install the project's own build backend; return extra pip args.
    ...
    environment, on every build. Installing the pinned backend here first,
    hash-verified via ``--require-hashes``, and returning
    ``--no-build-isolation`` makes the actual project install reuse it
    instead of pip resolving a fresh, unverified one into a throwaway
    isolated environment. A no-op (no install, no extra args) when
    ``build_pylock`` isn't configured.
    """
    ...
    if not build_pylock_path.exists():
        ...
        raise FileNotFoundError(msg)
    _log.info("Installing build backend (hash-verified): %s", build_pylock_path)
    subprocess.run(  # noqa: S603  # nosec B603
        [
            str(python_bin), "-m", "pip", "install", "--no-compile", "--no-deps",
            "--require-hashes", "-r", str(build_pylock_path),
        ],
        cwd=project_root,
        env=_pip_env(python_bin),
        check=True,
    )
...
def _install_from_pylock(
    ...
    subprocess.run(  # noqa: S603  # nosec B603
        [
            str(python_bin), "-m", "pip", "install", "--no-compile", "--no-deps",
            *_install_build_pylock(resolved, python_bin, project_root), *resolved.local_install_targets,
        ],
        cwd=project_root,
        env=_pip_env(python_bin),
        check=True,
    ...
    if resolved.pylock:
        ...
        subprocess.run(  # noqa: S603  # nosec B603
            [
                ...
                *_install_build_pylock(resolved, python_bin, project_root), *resolved.install_targets,
            ],
            cwd=project_root,
            env=_pip_env(python_bin),
            check=True,
