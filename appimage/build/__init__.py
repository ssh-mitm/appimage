...
def _pinned_download_fields(
    ...
    if "python_date" not in existing:
        _url, api_sha256, resolved_date = _resolve_python_url(resolved.python, resolved.python_date, arch)
...
def _resolve_python_url(python: str, date: str, arch: str) -> tuple[str, str | None, str]:
...
def _resolve_python_tarball(
    ...
    if python_cache.exists():
        ...
        return python_cache
    python_url, api_sha256, _resolved_date = _resolve_python_url(resolved.python, resolved.python_date, arch)
...
def _install_from_pylock(
    ...
    subprocess.run(  # noqa: S603  # nosec B603
        [
            ...
            *_install_build_pylock(resolved, project_root), *resolved.local_install_targets,
        ],
        cwd=project_root,
        check=True,
    )
...
def _resolve_appimage_pin_sha256(pin: str, *, strict: bool) -> str | None:
    ...
    *strict* (``resolved.verify_downloads``) asks for a hard failure
    ...
    try:
        ...
        _log.warning(
            "Could not verify %s against PyPI's published digest: %s. Installing unverified.", pin, exc,
        )
        return None
    ...
    if digest is None:
        if strict:
            ...
            raise RuntimeError(msg)
        _log.warning("PyPI has no published wheel digest for %s. Installing unverified.", pin)
    return digest


def _install_hashed_requirement(requirement: str, sha256: str, python_bin: Path, project_root: Path) -> None:
    ...
    command line.
    """
    with tempfile.NamedTemporaryFile("w", suffix=".txt", prefix="appimage-pin-") as req_file:
        req_file.write(f"{requirement} --hash=sha256:{sha256}\n")
        req_file.flush()
        _log.info("Installing %s (hash-verified against PyPI): sha256=%s", requirement, sha256)
        subprocess.run(  # noqa: S603  # nosec B603
            [
                str(python_bin), "-m", "pip", "install", "--no-compile", "--no-deps",
                "--require-hashes", "-r", req_file.name,
            ],
            cwd=project_root,
            check=True,
        )
...
def _install_targets(resolved: _ResolvedBuild, python_bin: Path, project_root: Path) -> None:
    ...
    ``appimage.build`` release itself — so it gets the same free
    ...
    """
    sha256 = _resolve_appimage_pin_sha256(resolved.appimage_pin, strict=resolved.verify_downloads)
    targets = resolved.install_targets
    if sha256:
        _install_hashed_requirement(resolved.appimage_pin, sha256, python_bin, project_root)
    ...
    subprocess.run(  # noqa: S603  # nosec B603
        [
            ...
            *_install_build_pylock(resolved, project_root), *targets,
        ],
        cwd=project_root,
        check=True,
    )
