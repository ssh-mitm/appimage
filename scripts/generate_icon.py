...
HERE = Path(__file__).parent.parent / "appimage" / "assets"
...
ARROW_SHAFT_W = 20  # shaft half-width x 2
...
# -5pi/8 ~= 11-o'clock; sin(8*angle)=0 -> head sits at base radius R
...
def body_path_d(pts: list[tuple[float, float]]) -> str:
    """Return the SVG path data string for the snake body."""
    return "M " + " L ".join(f"{x:.2f},{y:.2f}" for x, y in pts)


def tail_path_d(pts: list[tuple[float, float]]) -> str:
    ...
    tip_y = pts[0][1] - ty * 10
    return (
        f"M {pts[0][0] + px*w:.2f},{pts[0][1] + py*w:.2f} "
        f"L {tip_x:.2f},{tip_y:.2f} "
        f"L {pts[0][0] - px*w:.2f},{pts[0][1] - py*w:.2f}"
    )


def head_elements() -> tuple[str, str, float, float]:
    ...
    px, py = -oy, ox  # 90 degrees counter-clockwise
...
def arrow_path_d() -> str:
    """Return the SVG path data string for the downward arrow."""
    sw = ARROW_SHAFT_W / 2
    hw = ARROW_HEAD_W / 2
...
def build_svg() -> str:
    """Assemble and return the complete SVG document as a string."""
    pts = _body_points()
    body_d = body_path_d(pts)
    tail_d = tail_path_d(pts)
    head_d, _tongue_d, eye_x, eye_y = head_elements()
...
def main() -> None:
    """Generate the default AppImage icon SVG (and optionally a PNG)."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--png", action="store_true", help="also export PNG via cairosvg")
    args = parser.parse_args()

    svg_path = HERE / "default_icon.svg"
    svg_path.write_text(build_svg(), encoding="utf-8")
    print(f"Written: {svg_path}")  # noqa: T201
    ...
    if args.png:
        ...
        cairosvg.svg2png(
            url=str(svg_path), write_to=str(png_path), output_width=W, output_height=H
        )
        print(f"Written: {png_path}")  # noqa: T201
