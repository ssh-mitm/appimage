...
needs no configuration. Three further layers are opt-in on top of it, each
closing a different gap, each independent of the others. Run them in this
order to go from the zero-config default to everything pinned and
hash-verified:

1. **Pin the toolchain** — which Python, which appimagetool, which runtime
   stub — so a build next month or on another machine resolves the exact
   same bits instead of whatever is currently latest:
   ```bash
   python -m appimage.build --init
   ```
   Writes `python_date`, `appimagetool_sha256`, and `runtime_sha256` to
   `pyproject.toml`. Details: [Pinning for cross-machine
   reproducibility](#pinning-for-cross-machine-reproducibility).

2. **Hash-pin third-party dependencies** so a compromised or typosquatted
   package can't end up inside the AppImage unverified:
   ```bash
   python -m appimage.build --lock
   ```
   Writes `pylock.toml` and sets `pylock` in `pyproject.toml`. Details:
   [Verified dependencies](#verified-dependencies).

3. **Hash-pin the project's own build backend** — the one gap step 2
   deliberately leaves open, since a lock file can't pin the local project
   itself:
   ```toml
   [tool.appimage.build]
   build_constraint = "requirements-build.txt"
   ```
   `requirements-build.txt` is generated by hand, once per
   `[build-system].requires` change, not by `appimage.build` — see
   [Verified build backend](#verified-build-backend) for the exact command.

4. **Turn on the umbrella flag**, once steps 1–3 are in place:
   ```bash
   python -m appimage.build --reproducible
   ```
   Refuses to build unless the step 1 pins are set — see [Pinning for
...
Reproducibility checklist (1/3 ready):
  ...
  ✗ Build backend verification: build_constraint not set — the packaged project's own build backend is installed unverified
```

None of these three layers are required for the byte-identical guarantee
itself — they exist for projects that also need cross-machine/over-time
reproducibility (1) and supply-chain verification (2, 3), and each can be
adopted alone without the others.
