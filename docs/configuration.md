...
| `appimage_version` | *(the running `appimage.ctl` version)* | Exact version of the bundled `appimage` runtime module (the one AppRun depends on) to install, regardless of the packaged project's own declared dependencies. Left unset, it silently tracks whichever `appimage.ctl` release does the build; set explicitly (`init` does this automatically) to pin it in `pyproject.toml` instead. |
| `appimage_sha256` | - | Expected sha256 of the `appimage` wheel for `appimage_version`. When unset, the digest PyPI publishes for that release is looked up and used instead - set explicitly to verify without that lookup, e.g. for a fully offline build. |
| `appimagectl_version` | - | Expected version of `appimage.ctl` itself, the tool doing the build - not `appimage_version` above, which pins the bundled runtime module instead. Config-only, no CLI flag: the point is a committed value to compare *against*, not one a single invocation should be able to override. Empty skips the check. |
...
| `runtime_version` | - | Informational label recording which runtime release `runtime_sha256` corresponds to - the resolved release tag, since the runtime stub has no `--version` of its own the way appimagetool does. Written automatically by `init`. |
...
| `reproducible` | `false` | Shortcut for a build that's reproducible across machines and over time: implies `verify_downloads` and `require_zsyncmake`, and requires `python` (explicitly - `requires-python` in `[project]` doesn't count, it's a compatibility floor, not a build pin), `python_date` (or `python_dir`), `appimage_version`, `appimage_sha256`, `appimagetool_sha256`, and `runtime_sha256` to already be set (run `init` first - `reproducible` itself never resolves or writes values). Independent of `pylock`/`require_pylock` - opt into both explicitly. See [Getting to full reproducibility](reproducible-builds.md#getting-to-full-reproducibility). |
